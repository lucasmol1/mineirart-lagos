...
function renderPerformancePage(){
  ...
  const role=isAdmin1?'admin':(currentProfile?.viewPerformance==='manager'?'manager':'viewer');
  const nome=currentProfile?.name||'Usuário';
...
<aside class="sidebar">
  ...
  <div class="nav-item" id="nav-users" onclick="showPage('usuarios')"><span class="nav-icon">👤</span><span>Usuários</span></div>
...
function renderUsuarios(){
  if(!isAdmin()){document.getElementById('user-list').innerHTML=\`<div class="perm-warn">⚠ Apenas administradores podem gerenciar usuários.</div>\`;document.getElementById('btn-add-user').style.display='none';return;}
...
function salvarUser(){
  ...
  document.getElementById('sidebar-uname').textContent=SESSION?.nome||'';
  renderAll();
}
function deletarUser(id){
  ...
  saveDB();notify('Usuário removido');renderUsuarios();
}
function salvarSenha(){
  ...
  saveDB();closeModal('overlay-pwd');notify('Senha alterada');
}
...
window.addEventListener('message', function(e){
  ...
  const role = e.data.role || 'viewer';
  const nome = e.data.nome || 'Usuário';
  const id   = e.data.id   || 'u_ext';
  // Garante que o usuário existe no DB local
...
`;
  setTimeout(()=>{
    ...
    iframe.addEventListener('load',()=>{
      iframe.contentWindow.postMessage({type:'perf-login',role,nome,id},'*');
    },{once:true});
    iframe.srcdoc=PERF_HTML;
  },50);
