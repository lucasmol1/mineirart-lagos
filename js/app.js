...
function perfSaveDB(){localStorage.setItem('pos_db_v3',JSON.stringify(perfDB));}
...
function perfCanEdit(){return perfSESSION&&(SESSION.role==='admin'||SESSION.role==='manager');}
function perfIsAdmin(){return perfSESSION&&SESSION.role==='admin';}
...
// ============================================================
  document.getElementById('login-error').style.display='none';
  perfSESSION=u; window.SESSION=undefined;
...
function renderAll(){
  if(_page==='dashboard')perfRenderDashboard();
...
function salvarFuncionario(){
  ...
  if(eid){
    ...
    if(i>-1){perfDB.funcionarios[i]={...perfDB.funcionarios[i],...d};addLog('func_edit',`Funcionário editado: ${nome}`,`Equipe: ${d.equipe} | Cargo: ${d.cargo}`);}
...
function salvarFalha(){
  ...
  const ep=calcEffectivePeso(fid,mk,tipo);
  perfDB.falhas.push({id:perfUid(),func_id:fid,data,tipo,peso:ep,motivo,by_user:SESSION.id});
  addLog('fail_add',`Falha registrada: ${ft.label}`,`Funcionário: ${fn?.nome} | Data: ${perfFmtDate(data)} | Impacto: ${ep}pts | Motivo: ${motivo}`);
  perfSaveDB();perfCloseModal('overlay-falha');perfNotify('Falha registrada');renderAll();
}
function deletarFalha(id){
  ...
  perfSaveDB();perfNotify('Removido');renderFuncionarioDetail(window._did);
}
...
function salvarBonus(){
  ...
  perfDB.bonus.push({id:perfUid(),func_id:fid,data,tipo,valor:bt.valor,motivo,by_user:SESSION.id});
  addLog('bonus_add',`Bônus registrado: ${bt.label}`,`Funcionário: ${fn?.nome} | Data: ${perfFmtDate(data)} | Valor: +${bt.valor}pts | Motivo: ${motivo}`);
  perfSaveDB();perfCloseModal('overlay-bonus');perfNotify('Bônus registrado!','green');renderAll();
}
...
function renderUsuarios(){
  ...
  document.getElementById('user-list').innerHTML=perfDB.users.map(u=>{
    const uc=userColor(u.id);const isSelf=u.id===perfSESSION?.id;
    return`<div class="user-list-item">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        ${isSelf?`<button class="btn-ghost btn-sm" onclick="perfOpenModal('overlay-pwd')">🔒 Senha</button>`:''}
...
function openAddUser(){
  ...
  perfOpenModal('overlay-user');
}
function openEditUser(id){
  if(!perfIsAdmin())return;
  const u=perfDB.users.find(us=>us.id===id);if(!u)return;
...
function salvarUser(){
  ...
  if(eid){
    const i=perfDB.users.findIndex(u=>u.id===eid);
    if(i>-1){
      if(senha){if(senha.length<6){alert('Senha mín. 6 chars.');return;}perfDB.users[i].senha=senha;}
      perfDB.users[i]={...perfDB.users[i],nome,login,role};
      if(eid===perfSESSION?.id)perfSESSION={...SESSION,...perfDB.users[i]};
    ...
    if(perfDB.users.find(u=>u.login===login)){alert('Login já existe.');return;}
    const colors=['#5b6ef5','#22d3a0','#f5c842','#f55b5b','#a78bfa','#f5944a','#22c5f5'];
    perfDB.users.push({id:perfUid(),nome,login,senha,role,cor:colors[perfDB.users.length%colors.length]});
    addLog('user_add',`Usuário criado: ${nome}`,`Login: ${login} | Perfil: ${RL[role]}`);
    perfNotify('Usuário criado');
  }
  perfSaveDB();perfCloseModal('overlay-user');
  /* sidebar handled by main app */
  renderAll();
}
function deletarUser(id){
  if(!perfIsAdmin())return;
  const u=perfDB.users.find(us=>us.id===id);
  if(perfDB.users.length<=1){alert('Não é possível remover o único usuário.');return;}
  if(!confirm(`Remover "${u?.nome}"?`))return;
  addLog('user_delete',`Usuário removido: ${u?.nome}`,'');
  perfDB.users=perfDB.users.filter(us=>us.id!==id);
  perfSaveDB();perfNotify('Usuário removido');renderUsuarios();
}
function salvarSenha(){
  ...
  const i=perfDB.users.findIndex(u=>u.id===SESSION.id);
  if(i>-1){perfDB.users[i].senha=np;perfSESSION={...perfSESSION,senha:np};}
  perfSaveDB();perfCloseModal('overlay-pwd');perfNotify('Senha alterada');
}
...
function exportarJSON(){
  ...
  addLog('export','Exportação JSON','');perfSaveDB();perfNotify('JSON exportado');
}
...
function resetarDados(){
  ...
  const me=perfDB.users.find(u=>u.id===SESSION.id);
  ...
  setTimeout(()=>{
    if(window._perfLoaded){
      try{
        ...
        window.perfSESSION={id:currentUser?.uid||'u0',nome:currentProfile?.name||'Usuário',role:pRole};
        initMonthSelector();
        renderAll();
        document.querySelectorAll('#perf-wrap .overlay').forEach(el=>el.addEventListener('click',e=>{if(e.target===el)el.classList.remove('open');}));
