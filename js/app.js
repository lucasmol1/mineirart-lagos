...
// ── AUDIT ─────────────────────────────────────────────────────────────────────
function diffObj(before,after,labels){
  const changes=[];
  for(const[k,label] of Object.entries(labels)){
    const a=Array.isArray(before[k])?before[k].join(', '):String(before[k]||'');
    const b=Array.isArray(after[k])?after[k].join(', '):String(after[k]||'');
...
function initListeners(){
  ...
  dbListen(`users/${currentUser?.uid||'__none__'}`, v=>{ if(v&&currentProfile){const hadMA=currentProfile.manageAreas; currentProfile={...currentProfile,...v}; if(!!v.manageAreas!==!!hadMA)render();} else if(!v&&currentProfile){signOut(auth);}});
...
function renderTopbar(){
  ...
  tb.innerHTML=`<button id="hamburger-btn" aria-label="Menu">☰</button>
    ...
    <div style="position:relative">
      <div class="topbar-user" id="user-btn"><div class="user-avatar">${initials(currentProfile.name)}</div><span class="topbar-user-name">${esc(currentProfile.name)}</span><span style="font-size:10px;color:#c8f04e;margin-left:5px;font-weight:700">v1.49</span><span style="font-size:11px;color:#7a7a8a;margin-left:2px">▾</span></div>
...
const PERF_HTML_BASE=`<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>PerformanceOS v3</title>
...
<style>
:root{--bg:#0a0a0f;--surface:#12121a;--surface2:#1a1a26;--surface3:#22223a;--border:#2a2a3f;--accent:#5b6ef5;--accent2:#a78bfa;--green:#22d3a0;--yellow:#f5c842;--red:#f55b5b;--orange:#f5944a;--text:#e8e8f5;--text2:#9090b8;--text3:#5a5a80;--radius:12px;--radius2:8px;}
...
<div id="login-screen" style="display:none">
  <div class="login-box">
    <div class="login-logo">PerformanceOS</div>
    <div class="login-sub">v3.0 · acesso seguro</div>
    <div class="login-error" id="login-error">Usuário ou senha incorretos.</div>
    <div class="form-group"><label class="form-label">Usuário</label><input type="text" class="form-control" id="login-user" placeholder="seu.usuario" onkeydown="if(event.key==='Enter')doLogin()"></div>
    <div class="form-group"><label class="form-label">Senha</label><input type="password" class="form-control" id="login-pass" placeholder="••••••" onkeydown="if(event.key==='Enter')doLogin()"></div>
    <button class="btn-primary" style="width:100%;justify-content:center;margin-top:8px;" onclick="doLogin()">Entrar</button>
    <div style="margin-top:16px;font-size:11px;color:var(--text3);text-align:center;font-family:'IBM Plex Mono';">acesso inicial: admin / admin123</div>
  </div>
</div>
...
// ============================================================
// DB — sincronizado via Firebase REST API
// ============================================================
var _FB_URL='';
var _FB_TOKEN='';
...
function loadDB(){
  ...
  return{users:[],funcionarios:[],falhas:[],bonus:[],anotacoes:{},log:[]};
}
let DB=loadDB();
let SESSION=null;
function saveDB(){
  try{localStorage.setItem('pos_db_v3',JSON.stringify(DB));}catch(e){}
  if(_FB_URL&&_FB_TOKEN){
    fetch(_FB_URL+'/performance.json?auth='+_FB_TOKEN,{
      method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(DB)
    }).catch(function(){});
  }
}
function loadFromFirebase(callback){
  if(!_FB_URL||!_FB_TOKEN){callback&&callback();return;}
  fetch(_FB_URL+'/performance.json?auth='+_FB_TOKEN)
    .then(function(r){return r.json();})
    .then(function(data){
      if(data&&!data.error){
        // Mescla — preserva estrutura mesmo se algum campo vier null
        if(data.funcionarios) DB.funcionarios=data.funcionarios;
        if(data.falhas)       DB.falhas=data.falhas;
        if(data.bonus)        DB.bonus=data.bonus;
        if(data.anotacoes)    DB.anotacoes=data.anotacoes;
        if(data.log)          DB.log=data.log;
...
function uid(){return 'id'+Date.now()+Math.random().toString(36).slice(2,7);}function getMonthKey(d){return d.getFullYear()+'-'+(String(d.getMonth()+1).padStart(2,'0'));}
...
function canEdit(){return SESSION&&(SESSION.role==='admin'||SESSION.role==='manager');}
...
function addLog(type,action,detail){
  DB.log.unshift({id:uid(),ts:new Date().toISOString(),type,action,detail,user_id:SESSION?.id||'system',user_nome:SESSION?.nome||'Sistema'});
...
function doLogin(){
  ...
  const u=DB.users.find(u=>u.login===login&&u.senha===senha);
  ...
  document.getElementById('recent-fails').innerHTML=allRecs.length?allRecs.map(r=>{
    ...
    if(r.kind==='bonus'){
      ...
      return\`<div class="fail-item" style="cursor:pointer" onclick="openDetail('\${r.func_id}')">
        <div class="fail-date">\${fmtDate(r.data)}</div>
...
function renderFuncionarioDetail(id){
  ...
  const recsHtml=allRecs.length?allRecs.map(r=>{
    if(r.kind==='bonus'){
      const bt=getBT(r.tipo);const bu=getUserById(r.by_user);const ce=canEdit();
      return\`<div class="fail-item"><div class="fail-date">\${fmtDate(r.data)}</div>
    ...
    const ft=getFT(r.tipo);const bu=getUserById(r.by_user);const ce=canEdit();
  ...
  const eb=canEdit()?\`<button class="btn-ghost btn-sm" onclick="openEditFuncionario('\${id}')">✏ Editar</button>\`:'';
...
function renderFuncionarios(){
  document.getElementById('btn-add-func').style.display=canEdit()?'flex':'none';
  ...
  document.getElementById('employee-grid').innerHTML=DB.funcionarios.map(f=>{
    ...
    return\`<div class="employee-card">
      ...
      <div style="display:flex;gap:8px">
        ...
        \${canEdit()?\`<button class="btn-warn btn-sm" onclick="openEditFuncionario('\${f.id}')">✏</button>\`:''}
        \${isAdmin()?\`<button class="btn-danger btn-sm" onclick="deletarFuncionario('\${f.id}')">✕</button>\`:''}
      </div></div>\`;
  }).join('');
}
function openAddFuncionario(){
  if(!canEdit()){notify('Sem permissão');return;}
...
function deletarFuncionario(id){
  ...
  saveDB();notify('Removido');renderFuncionarios();
}
...
function salvarFalha(){
  ...
  const ep=calcEffectivePeso(fid,mk,tipo);
  DB.falhas.push({id:uid(),func_id:fid,data,tipo,peso:ep,motivo,by_user:SESSION.id});
  addLog('fail_add',\`Falha registrada: \${ft.label}\`,\`Funcionário: \${fn?.nome} | Data: \${fmtDate(data)} | Impacto: \${ep}pts | Motivo: \${motivo}\`);
  saveDB();closeModal('overlay-falha');notify('Falha registrada');loadFromFirebase(function(){renderAll();});
}
function deletarFalha(id){
  ...
  addLog('fail_delete',\`Falha removida: \${ft?.label||f?.tipo}\`,\`Funcionário: \${fn?.nome} | Data: \${fmtDate(f?.data||'')} | Motivo: \${f?.motivo}\`);
  DB.falhas=DB.falhas.filter(fl=>fl.id!==id);
  saveDB();loadFromFirebase(function(){notify('Removido');renderFuncionarioDetail(window._did);});
}
...
function salvarBonus(){
  ...
  DB.bonus.push({id:uid(),func_id:fid,data,tipo,valor:bt.valor,motivo,by_user:SESSION.id});
  addLog('bonus_add',\`Bônus registrado: \${bt.label}\`,\`Funcionário: \${fn?.nome} | Data: \${fmtDate(data)} | Valor: +\${bt.valor}pts | Motivo: \${motivo}\`);
  saveDB();closeModal('overlay-bonus');notify('Bônus registrado!','green');loadFromFirebase(function(){renderAll();});
}
function deletarBonus(id){
  ...
  saveDB();notify('Bônus removido');renderFuncionarioDetail(window._did);
}
...
function exportarJSON(){
  ...
  addLog('export','Exportação JSON','');saveDB();notify('JSON exportado');
}
...
function handleImport(e){
  ...
  const r=new FileReader();r.onload=ev=>{
    try{
      ...
      saveDB();notify('Dados importados');renderAll();
...
function exportarCSV(){
  ...
  const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='perf_'+mk+'.csv';a.click();notify('CSV exportado');
}
function resetarDados(){
  if(!isAdmin()){notify('Apenas admins');return;}
  ...
  saveDB();notify('Dados resetados');renderAll();
}
...
// ============================================================
(function(){
  var s = (typeof _INJECTED_SESSION !== 'undefined') ? _INJECTED_SESSION
        : {id:'fallback',nome:'Admin',login:'admin',senha:'admin123',role:'admin',cor:'#5b6ef5'};
  // Recebe URL e token do Firebase passados pelo sistema principal
...
function renderPerformancePage(){
  ...
  const _pNome=(currentProfile?.name||'Usuário').replace(/"/g,'\\"');
  const _pId=(currentUser?.uid||'u_ext').replace(/"/g,'\\"');
  const _buildHtml=(token)=>{
    const _inj='<script>var _INJECTED_SESSION='+JSON.stringify({
      id:_pId,nome:_pNome,login:_pId,senha:'__ext__',role:'admin',cor:'#5b6ef5',
      dbUrl:'https://mineirart-lagos-default-rtdb.firebaseio.com',token:token||""
    })+';<\/script>';
    return PERF_HTML_BASE.replace('</head>',_inj+'</head>');
  };
  const _setIframe=(html)=>{
    setTimeout(()=>{ const f=document.getElementById('perf-iframe'); if(f) f.srcdoc=html; },30);
  };
  if(typeof auth!=="undefined"&&auth.currentUser){
    auth.currentUser.getIdToken().then(t=>_setIframe(_buildHtml(t))).catch(()=>_setIframe(_buildHtml("")));
  } else {
    _setIframe(_buildHtml(""));
  }
  return '<div style="width:100%;height:calc(100vh - 56px);overflow:hidden"><iframe id="perf-iframe" style="width:100%;height:100%;border:none;background:#0a0a0f"></iframe></div>';
}
...
function attachContentEvents(){
  ...
  document.querySelectorAll(".btn-del-note").forEach(b=>b.addEventListener("click",async()=>{if(confirm("Excluir nota?")){const _nd=notes[b.dataset.id];await dbRemove(`notes/${b.dataset.id}`);await logAction("excluir_nota",`Nota excluída: "${_nd?.title||'?'}"`);toast("Nota excluída","warning");}}));
  ...
  document.querySelectorAll(".save-user-name").forEach(b=>b.addEventListener("click",async()=>{
    ...
    await logAction("renomear_usuario",`Renomeado: "${users[currentUser.uid]?.name}" → "${name}"`);
  ...
  document.querySelectorAll(".toggle-manage-areas").forEach(b=>b.addEventListener("click",async()=>{
    ...
    await logAction("permissao_areas",`Permissão gerenciar áreas ${newVal?'concedida a':'removida de'} "${u.name}"`);
...
function renderProspPage(){
  ...
  for(let i=0;i<totalCells;i++){
    ...
    const isPast=isCurrentMonth&&cellDate<today;
    const evs=eventMap[ds]||[];
    const chips=evs.map(e=>`<div class="prosp-chip" data-eid="${e.id}" style="background:${e.color||"#7c6eff"}22;border-left:3px solid ${e.color||"#7c6eff"};color:${e.color||"#7c6eff"};font-size:11px;padding:2px 5px;border-radius:3px;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer;max-width:100%">${e.isFirst||!e.span?"🎯 "+esc(e.title.slice(0,15)):"↳ "+esc(e.title.slice(0,12))}</div>`).join("");
...
function openEditAreaModal(areaId){
  ...
  document.getElementById("m-save").onclick=async()=>{
    ...
    await logAction("editar_area",`Área editada: "${a.name}" → "${name}"${a.color!==col?` | Cor: "${a.color}" → "${col}"`:''}`);
...
function openTaskModal(init={}){
  ...
  document.getElementById("m-save").onclick=async()=>{
    ...
    const _before=isEdit?{...tasks[data.id]}:{};
    await dbSet(`tasks/${data.id}`,data);
    if(isEdit){
      const _diff=diffObj(_before,data,{title:'Título',desc:'Descrição',status:'Status',priority:'Prioridade',date:'Prazo',areaId:'Área',resps:'Responsáveis'});
      await logAction("editar_tarefa",`Editada por ${currentProfile.name}: ${title} | ${_diff}`);
    } else {
      await logAction("criar_tarefa",`Criada por ${currentProfile.name}: ${title}`);
    }
  ...
  document.getElementById("comment-submit")?.addEventListener("click",async()=>{
    ...
    for(const uid2 of toNotify){
      ...
      await dbSet(`user_notifs/${uid2}/${uid()}`,{
        ...
        taskId, commentAuthor:currentProfile.name, commentPreview, mention:isMentioned,
