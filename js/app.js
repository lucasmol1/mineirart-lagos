...
// HTML do módulo Performance (carregado sob demanda)
...
// ============================================================
// INIT — SESSION injetado diretamente pelo sistema principal
// ============================================================
(function(){
  var s = (typeof _INJECTED_SESSION !== 'undefined') ? _INJECTED_SESSION : {id:'fallback',nome:'Usuário',login:'u',senha:'',role:'admin',cor:'#5b6ef5'};
  var u = DB.users.find(function(us){ return us.id === s.id; });
  if(!u){
    u = {id:s.id, nome:s.nome, login:s.login, senha:s.senha, role:s.role, cor:s.cor};
    DB.users.push(u); saveDB();
  } else {
    u.role = s.role; u.nome = s.nome; saveDB();
  }
  SESSION = u;
...
function renderPerformancePage(){
  ...
  // Injeta SESSION diretamente no HTML — sem depender de postMessage
  const _injScript=`<script>var _INJECTED_SESSION={id:'${_pId}',nome:'${_pNome}',login:'${_pId}',senha:'__ext__',role:'admin',cor:'#5b6ef5'};<\/script>`;
  const _finalHtml=PERF_HTML_BASE.replace('</head>',_injScript+'</head>');
  setTimeout(()=>{
    const _ifr=document.getElementById('perf-iframe');
    if(_ifr) _ifr.srcdoc=_finalHtml;
  },30);
  return '<div style="width:100%;height:calc(100vh - 56px);overflow:hidden"><iframe id="perf-iframe" style="width:100%;height:100%;border:none;background:#0a0a0f" sandbox="allow-scripts allow-same-origin allow-downloads allow-modals allow-forms"></iframe></div>';
}
