...
function renderTopbar(){
  ...
  tb.innerHTML=`<button id="hamburger-btn" aria-label="Menu">☰</button>
    ...
    <div style="position:relative">
      <div class="topbar-user" id="user-btn"><div class="user-avatar">${initials(currentProfile.name)}</div><span class="topbar-user-name">${esc(currentProfile.name)}</span><span style="font-size:10px;color:#c8f04e;margin-left:5px;font-weight:700">v1.2</span><span style="font-size:11px;color:#7a7a8a;margin-left:2px">▾</span></div>
...
// ══════════════════════════════════════════════════════════════════
//  PERFORMANCE MODULE v1
// ══════════════════════════════════════════════════════════════════

(function(){
  if(document.getElementById("perf-style"))return;
  const s=document.createElement("style");
  s.id="perf-style";
  s.textContent=":root{--bg:#0a0a0f;--surface:#12121a;--surface2:#1a1a26;--surface3:#22223a;--border:#2a2a3f;--accent:#5b6ef5;--accent2:#a78bfa;--green:#22d3a0;--yellow:#f5c842;--red:#f55b5b;--orange:#f5944a;--text:#e8e8f5;--text2:#9090b8;--text3:#5a5a80;--radius:12px;--radius2:8px;}\n\n#perf-container * {margin:0;padding:0;box-sizing:border-box;}\n#perf-container body {font-family:'Inter',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;font-size:14px;}\n#perf-container body::before {content:'';position:fixed;inset:0;background-image:linear-gradient(rgba(91,110,245,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(91,110,245,0.03) 1px,transparent 1px);background-size:40px 40px;pointer-events:none;z-index:0;}\n#perf-container .app {position:relative;z-index:1;}\n#perf-container #login-screen {display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px;position:relative;z-index:1;}\n#perf-container .login-box {background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:40px;width:400px;max-width:100%;}\n#perf-container .login-logo {font-family:'Syne',sans-serif;font-weight:800;font-size:24px;background:linear-gradient(135deg,var(--accent),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:4px;}\n#perf-container .login-sub {font-size:12px;color:var(--text3);font-family:'IBM Plex Mono';margin-bottom:32px;}\n#perf-container .login-error {background:rgba(245,91,91,0.1);border:1px solid rgba(245,91,91,0.25);border-radius:8px;padding:10px 14px;font-size:13px;color:var(--red);margin-bottom:16px;display:none;}\n#perf-container .sidebar {position:fixed;left:0;top:0;bottom:0;width:225px;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;padding:24px 0;z-index:100;}\n#perf-container .logo {padding:0 20px 24px;border-bottom:1px solid var(--border);margin-bottom:16px;}\n#perf-container .logo-title {font-family:'Syne',sans-serif;font-weight:800;font-size:18px;letter-spacing:-0.5px;background:linear-gradient(135deg,var(--accent),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;}\n#perf-container .logo-sub {font-size:11px;color:var(--text3);font-family:'IBM Plex Mono';margin-top:2px;}\n#perf-container .nav-item {display:flex;align-items:center;gap:10px;padding:10px 20px;cursor:pointer;color:var(--text2);font-size:13px;font-weight:500;border-left:3px solid transparent;transition:all 0.15s;}\n#perf-container .nav-item:hover {color:var(--text);background:var(--surface2);}\n#perf-container .nav-item.active {color:var(--accent);border-left-color:var(--accent);background:rgba(91,110,245,0.07);}\n#perf-container .nav-icon {font-size:15px;width:20px;text-align:center;}\n#perf-container .nav-section {font-size:10px;color:var(--text3);font-family:'IBM Plex Mono';padding:16px 20px 6px;letter-spacing:0.1em;text-transform:uppercase;}\n#perf-container .sidebar-user {margin-top:auto;padding:16px 20px;border-top:1px solid var(--border);}\n#perf-container .sidebar-user-name {font-size:13px;font-weight:600;color:var(--text);}\n#perf-container .sidebar-user-role {font-size:11px;color:var(--text3);font-family:'IBM Plex Mono';margin-top:2px;}\n#perf-container .btn-logout {margin-top:10px;width:100%;background:transparent;border:1px solid var(--border);border-radius:var(--radius2);padding:7px;font-size:12px;color:var(--text3);cursor:pointer;transition:all 0.15s;}\n#perf-container .btn-logout:hover {color:var(--red);border-color:rgba(245,91,91,0.4);}\n#perf-container .main {margin-left:225px;min-height:100vh;}\n#perf-container .topbar {height:60px;background:var(--surface);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;padding:0 28px;position:sticky;top:0;z-index:90;}\n#perf-container .topbar-title {font-family:'Syne',sans-serif;font-weight:700;font-size:17px;letter-spacing:-0.3px;}\n#perf-container .month-selector {display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius2);padding:6px 12px;font-size:13px;color:var(--text);font-family:'IBM Plex Mono';}\n#perf-container .month-selector select {background:transparent;border:none;color:var(--text);font-family:'IBM Plex Mono';font-size:13px;cursor:pointer;outline:none;}\n#perf-container .btn-primary {background:linear-gradient(135deg,var(--accent),#7b8ef5);color:#fff;border:none;border-radius:var(--radius2);padding:8px 16px;font-size:13px;font-weight:600;cursor:pointer;transition:opacity 0.15s;display:flex;align-items:center;gap:6px;}\n#perf-container .btn-primary:hover {opacity:0.88;}\n#perf-container .btn-ghost {background:transparent;color:var(--text2);border:1px solid var(--border);border-radius:var(--radius2);padding:7px 14px;font-size:13px;cursor:pointer;transition:all 0.15s;}\n#perf-container .btn-ghost:hover {color:var(--text);border-color:var(--accent);}\n#perf-container .btn-danger {background:rgba(245,91,91,0.12);color:var(--red);border:1px solid rgba(245,91,91,0.25);border-radius:var(--radius2);padding:6px 12px;font-size:12px;cursor:pointer;transition:all 0.15s;}\n#perf-container .btn-danger:hover {background:rgba(245,91,91,0.22);}\n#perf-container .btn-warn {background:rgba(245,148,74,0.12);color:var(--orange);border:1px solid rgba(245,148,74,0.25);border-radius:var(--radius2);padding:6px 12px;font-size:12px;cursor:pointer;transition:all 0.15s;}\n#perf-container .btn-warn:hover {background:rgba(245,148,74,0.22);}\n#perf-container .btn-green {background:rgba(34,211,160,0.12);color:var(--green);border:1px solid rgba(34,211,160,0.25);border-radius:var(--radius2);padding:6px 12px;font-size:12px;cursor:pointer;transition:all 0.15s;}\n#perf-container .btn-green:hover {background:rgba(34,211,160,0.22);}\n#perf-container .content {padding:24px 28px;}\n#perf-container .card {background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:20px;}\n#perf-container .card-title {font-family:'Syne',sans-serif;font-weight:700;font-size:13px;color:var(--text2);letter-spacing:0.04em;margin-bottom:16px;text-transform:uppercase;}\n#perf-container .stats-grid {display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:24px;}\n#perf-container .stat-card {background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:18px;}\n#perf-container .stat-label {font-size:11px;color:var(--text3);font-family:'IBM Plex Mono';text-transform:uppercase;letter-spacing:0.08em;margin-bottom:8px;}\n#perf-container .stat-value {font-family:'Syne',sans-serif;font-weight:800;font-size:28px;letter-spacing:-1px;}\n#perf-container .stat-sub {font-size:11px;color:var(--text3);margin-top:4px;}\n#perf-container .badge {display:inline-flex;align-items:center;gap:5px;padding:3px 9px;border-radius:20px;font-size:11px;font-weight:600;font-family:'IBM Plex Mono';}\n#perf-container .badge-green {background:rgba(34,211,160,0.12);color:var(--green);border:1px solid rgba(34,211,160,0.2);}\n#perf-container .badge-yellow {background:rgba(245,200,66,0.12);color:var(--yellow);border:1px solid rgba(245,200,66,0.2);}\n#perf-container .badge-red {background:rgba(245,91,91,0.12);color:var(--red);border:1px solid rgba(245,91,91,0.2);}\n#perf-container .badge-blue {background:rgba(91,110,245,0.12);color:var(--accent);border:1px solid rgba(91,110,245,0.2);}\n#perf-container .score-dot {width:7px;height:7px;border-radius:50%;}\n#perf-container .dot-green {background:var(--green);}.dot-yellow{background:var(--yellow);}.dot-red{background:var(--red);}\n#perf-container .table-wrap {overflow-x:auto;}\n#perf-container table {width:100%;border-collapse:collapse;}\n#perf-container th {text-align:left;padding:10px 12px;font-size:11px;color:var(--text3);font-family:'IBM Plex Mono';text-transform:uppercase;letter-spacing:0.06em;border-bottom:1px solid var(--border);font-weight:500;white-space:nowrap;}\n#perf-container td {padding:12px 12px;border-bottom:1px solid rgba(42,42,63,0.6);vertical-align:middle;}\n#perf-container tr:last-child td {border-bottom:none;}\n#perf-container tr:hover td {background:rgba(91,110,245,0.03);}\n#perf-container .td-name {font-weight:600;font-size:14px;cursor:pointer;color:var(--text);}\n#perf-container .td-name:hover {color:var(--accent);}\n#perf-container .score-cell {font-family:'Syne',sans-serif;font-weight:800;font-size:18px;}\n#perf-container .trend {font-family:'IBM Plex Mono';font-size:12px;}\n#perf-container .trend-up {color:var(--green);}.trend-down{color:var(--red);}.trend-flat{color:var(--text3);}\n#perf-container .fail-count {font-family:'IBM Plex Mono';font-size:13px;color:var(--text2);}\n#perf-container .fail-zero {color:var(--text3);}\n#perf-container .btn-sm {padding:5px 10px;font-size:12px;}\n#perf-container .overlay {position:fixed;inset:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:1000;backdrop-filter:blur(4px);opacity:0;pointer-events:none;transition:opacity 0.2s;}\n#perf-container .overlay.open {opacity:1;pointer-events:all;}\n#perf-container .modal {background:var(--surface2);border:1px solid var(--border);border-radius:16px;padding:28px;width:500px;max-width:96vw;max-height:90vh;overflow-y:auto;transform:translateY(12px);transition:transform 0.2s;}\n#perf-container .overlay.open .modal {transform:translateY(0);}\n#perf-container .modal-title {font-family:'Syne',sans-serif;font-weight:800;font-size:18px;margin-bottom:20px;}\n#perf-container .form-group {margin-bottom:16px;}\n#perf-container .form-label {display:block;font-size:11px;color:var(--text3);font-family:'IBM Plex Mono';text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px;}\n#perf-container .form-control {width:100%;background:var(--surface3);border:1px solid var(--border);border-radius:var(--radius2);padding:10px 12px;color:var(--text);font-size:14px;font-family:'Inter';outline:none;transition:border-color 0.15s;}\n#perf-container .form-control:focus {border-color:var(--accent);}\n#perf-container .form-control option {background:var(--surface3);}\n#perf-container .modal-actions {display:flex;gap:10px;justify-content:flex-end;margin-top:20px;}\n#perf-container .form-row {display:grid;grid-template-columns:1fr 1fr;gap:14px;}\n#perf-container .detail-header {display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:24px;gap:16px;flex-wrap:wrap;}\n#perf-container .detail-name {font-family:'Syne',sans-serif;font-weight:800;font-size:26px;letter-spacing:-0.5px;}\n#perf-container .detail-score {font-family:'Syne',sans-serif;font-weight:800;font-size:48px;letter-spacing:-2px;line-height:1;}\n#perf-container .score-green {color:var(--green);}.score-yellow{color:var(--yellow);}.score-red{color:var(--red);}\n#perf-container .chart-container {position:relative;height:140px;margin-bottom:4px;}\n#perf-container canvas {display:block;}\n#perf-container .fail-item {display:flex;gap:14px;align-items:flex-start;padding:12px 0;border-bottom:1px solid rgba(42,42,63,0.6);}\n#perf-container .fail-item:last-child {border-bottom:none;}\n#perf-container .fail-date {font-family:'IBM Plex Mono';font-size:11px;color:var(--text3);white-space:nowrap;padding-top:2px;}\n#perf-container .fail-body {flex:1;}\n#perf-container .fail-type {font-size:13px;font-weight:600;color:var(--text);}\n#perf-container .fail-reason {font-size:12px;color:var(--text3);margin-top:2px;}\n#perf-container .fail-by {font-size:11px;color:var(--text3);margin-top:3px;font-family:'IBM Plex Mono';}\n#perf-container .fail-impact {font-family:'Syne',sans-serif;font-weight:700;font-size:14px;white-space:nowrap;}\n#perf-container .fail-impact.neg {color:var(--red);}\n#perf-container .fail-impact.pos {color:var(--green);}\n#perf-container .fail-impact-btn {display:flex;align-items:center;gap:8px;}\n#perf-container .chips-row {display:flex;flex-wrap:wrap;gap:8px;}\n#perf-container .chip {background:var(--surface3);border:1px solid var(--border);border-radius:20px;padding:5px 12px;font-size:12px;font-family:'IBM Plex Mono';display:flex;align-items:center;gap:6px;}\n#perf-container .chip-count {font-weight:700;}.chip-label{color:var(--text2);}\n#perf-container .chip-neg .chip-count {color:var(--red);}\n#perf-container .chip-pos .chip-count {color:var(--green);}\n#perf-container .notes-area {width:100%;background:var(--surface3);border:1px solid var(--border);border-radius:var(--radius2);padding:12px;color:var(--text);font-size:13px;font-family:'Inter';outline:none;resize:vertical;min-height:100px;transition:border-color 0.15s;line-height:1.6;}\n#perf-container .notes-area:focus {border-color:var(--accent);}\n#perf-container .grid-2 {display:grid;grid-template-columns:1fr 1fr;gap:16px;}\n#perf-container .page {display:none;}.page.active{display:block;}\n#perf-container .empty {text-align:center;padding:40px;color:var(--text3);}\n#perf-container .empty-icon {font-size:36px;margin-bottom:12px;}.empty-text{font-size:14px;}\n#perf-container .filter-row {display:flex;gap:10px;margin-bottom:18px;flex-wrap:wrap;}\n#perf-container .filter-select {background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius2);padding:7px 12px;color:var(--text);font-size:13px;font-family:'IBM Plex Mono';outline:none;cursor:pointer;}\n#perf-container .filter-select option {background:var(--surface2);}\n#perf-container .topbar-actions {display:flex;gap:10px;align-items:center;}\n#perf-container ::-webkit-scrollbar {width:6px;height:6px;}::-webkit-scrollbar-track{background:var(--bg);}::-webkit-scrollbar-thumb{background:var(--border);border-radius:3px;}\n#perf-container .notif {position:fixed;bottom:24px;right:24px;background:var(--surface2);border:1px solid var(--accent);border-radius:var(--radius);padding:12px 18px;font-size:13px;color:var(--text);transform:translateY(20px);opacity:0;transition:all 0.3s;z-index:2000;display:flex;align-items:center;gap:8px;box-shadow:0 8px 30px rgba(91,110,245,0.25);}\n#perf-container .notif.show {transform:translateY(0);opacity:1;}\n/* Employee grid */\n#perf-container .employee-grid {display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;}\n#perf-container .employee-card {background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:16px;display:flex;flex-direction:column;gap:10px;}\n#perf-container .employee-card-name {font-family:'Syne',sans-serif;font-weight:700;font-size:15px;}\n#perf-container .employee-card-team {font-size:11px;color:var(--text3);font-family:'IBM Plex Mono';}\n#perf-container .sep {height:1px;background:var(--border);margin:4px 0;}\n#perf-container .score-bar-wrap {height:6px;background:var(--surface3);border-radius:3px;overflow:hidden;margin-top:4px;}\n#perf-container .score-bar-fill {height:100%;border-radius:3px;transition:width 0.5s;}\n/* Log */\n#perf-container .log-item {display:flex;gap:14px;padding:12px 0;border-bottom:1px solid rgba(42,42,63,0.5);align-items:flex-start;}\n#perf-container .log-item:last-child {border-bottom:none;}\n#perf-container .log-time {font-family:'IBM Plex Mono';font-size:10px;color:var(--text3);white-space:nowrap;padding-top:3px;min-width:80px;}\n#perf-container .log-icon {width:28px;height:28px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;}\n#perf-container .lic-fail {background:rgba(245,91,91,0.15);}\n#perf-container .lic-del {background:rgba(245,148,74,0.15);}\n#perf-container .lic-add {background:rgba(34,211,160,0.15);}\n#perf-container .lic-edit {background:rgba(91,110,245,0.15);}\n#perf-container .lic-login {background:rgba(167,139,250,0.15);}\n#perf-container .lic-note {background:rgba(245,200,66,0.15);}\n#perf-container .lic-bonus {background:rgba(34,211,160,0.15);}\n#perf-container .log-body {flex:1;}\n#perf-container .log-action {font-size:13px;font-weight:600;color:var(--text);}\n#perf-container .log-detail {font-size:12px;color:var(--text3);margin-top:2px;line-height:1.5;}\n#perf-container .log-user {display:inline-flex;align-items:center;gap:5px;background:var(--surface3);border:1px solid var(--border);border-radius:12px;padding:2px 8px;font-size:11px;font-family:'IBM Plex Mono';color:var(--text2);}\n/* Users */\n#perf-container .user-list-item {display:flex;align-items:center;gap:14px;padding:14px 0;border-bottom:1px solid rgba(42,42,63,0.5);}\n#perf-container .user-list-item:last-child {border-bottom:none;}\n#perf-container .user-avatar {width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;font-family:'Syne',sans-serif;flex-shrink:0;}\n#perf-container .user-info {flex:1;}\n#perf-container .user-name-t {font-size:14px;font-weight:600;}\n#perf-container .user-meta {font-size:11px;color:var(--text3);font-family:'IBM Plex Mono';margin-top:2px;}\n#perf-container .rb {display:inline-flex;padding:2px 8px;border-radius:10px;font-size:10px;font-family:'IBM Plex Mono';font-weight:600;}\n#perf-container .rb-admin {background:rgba(91,110,245,0.15);color:var(--accent);}\n#perf-container .rb-manager {background:rgba(34,211,160,0.15);color:var(--green);}\n#perf-container .rb-viewer {background:rgba(144,144,184,0.15);color:var(--text2);}\n#perf-container .perm-warn {background:rgba(245,148,74,0.08);border:1px solid rgba(245,148,74,0.2);border-radius:8px;padding:10px 14px;font-size:13px;color:var(--orange);}\n/* Scoring info box */\n#perf-container .score-info-box {background:var(--surface3);border:1px solid var(--border);border-radius:var(--radius2);padding:14px 16px;font-size:12px;color:var(--text3);line-height:1.8;margin-bottom:16px;}\n#perf-container .score-info-box b {color:var(--text2);}\n#perf-container .bonus-row {background:rgba(34,211,160,0.06);border:1px solid rgba(34,211,160,0.15);border-radius:8px;padding:10px 14px;display:flex;align-items:center;gap:10px;font-size:13px;}\n#perf-container .cap-tag {display:inline-flex;align-items:center;background:rgba(245,200,66,0.12);color:var(--yellow);border:1px solid rgba(245,200,66,0.2);border-radius:10px;padding:1px 7px;font-size:10px;font-family:'IBM Plex Mono';margin-left:6px;}\n@media(max-width:900px){.sidebar{width:52px;}.nav-item span,.logo-title,.logo-sub,.nav-section,.sidebar-user-name,.sidebar-user-role,.btn-logout{display:none;}.main{margin-left:52px;}.stats-grid{grid-template-columns:1fr 1fr;}.grid-2{grid-template-columns:1fr;}}";
  document.head.appendChild(s);
})();

let perfDB={users:[],funcionarios:[],falhas:[],bonus:[],anotacoes:{},log:[]};
let perfSESSION=null;

function perfSaveDB(){
  if(typeof auth==="undefined"||!auth.currentUser)return;
  auth.currentUser.getIdToken().then(function(t){
    fetch("https://mineirart-lagos-default-rtdb.firebaseio.com/performance.json?auth="+t,{
      method:"PUT",headers:{"Content-Type":"application/json"},
      body:JSON.stringify(perfDB)
    }).catch(function(){});
  }).catch(function(){});
}

function perfLoadFromFirebase(cb){
  if(typeof auth==="undefined"||!auth.currentUser){cb&&cb();return;}
  auth.currentUser.getIdToken().then(function(t){
    fetch("https://mineirart-lagos-default-rtdb.firebaseio.com/performance.json?auth="+t)
      .then(function(r){return r.json();})
      .then(function(d){
        if(d&&!d.error){
          if(d.funcionarios)perfDB.funcionarios=d.funcionarios;
          if(d.falhas)      perfDB.falhas=d.falhas;
          if(d.bonus)       perfDB.bonus=d.bonus;
          if(d.anotacoes)   perfDB.anotacoes=d.anotacoes;
          if(d.log)         perfDB.log=d.log;
          if(d.users)       perfDB.users=d.users;
        }
        cb&&cb();
...
function perfInit(){
  const nome=(currentProfile?.name||"Admin").replace(/"/g,'\\"');
  const id=currentUser?.uid||"u_ext";
  let u=perfDB.users.find(function(us){return us.id===id;});
  if(!u){u={id:id,nome:nome,login:id,senha:"__ext__",role:"admin",cor:"#5b6ef5"};perfDB.users.push(u);}
  else{u.role="admin";u.nome=nome;}
  perfSESSION=u;
  const _un=document.getElementById('sidebar-uname');const _ur=document.getElementById('sidebar-urole');
  if(_un)_un.textContent=nome;if(_ur)_ur.textContent='Admin';
}
...
const BONUS_TYPES = [
  ...
  {id:'disponibilidade', label:'Disponibilidade',        valor:+2, cap:+4,
   desc:'+2pts por disponibilidade. Máx +4pts/mês.'},
  ...
  {id:'ideia_aprovada',  label:'Ideia aprovada',              valor:+3, cap:+9,
   desc:'+3pts por ideia aprovada e implementada. Máx +9pts/mês.'},
];
...
// ============================================================
// DB — gerenciado pelo app.js principal via perfDB / perfSESSION
// ============================================================
// perfDB e perfSESSION são injetados pelo renderPerformancePage

function perfUid(){return 'id'+Date.now()+Math.random().toString(36).slice(2,7);}function getMonthKey(d){return d.getFullYear()+'-'+(String(d.getMonth()+1).padStart(2,'0'));}
function monthLabel(k){const[y,m]=k.split('-');return['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][parseInt(m)-1]+'/'+y;}
function curMonth(){
  const sel=document.getElementById('month-global');
  if(sel&&sel.value)return sel.value;
  return getMonthKey(new Date()); // fallback: mês atual
}
...
function perfCanEdit(){return perfSESSION&&(perfSESSION.role==='admin'||perfSESSION.role==='manager');}
function perfIsAdmin(){return perfSESSION&&perfSESSION.role==='admin';}
function getUserById(id){return perfDB.users.find(u=>u.id===id);}
...
function userColor(uid){
  ...
  const i=perfDB.users.findIndex(u=>u.id===uid);return c[Math.max(0,i)%c.length];
}
...
// LOG
// ============================================================
function perfAddLog(type,action,detail){
  perfDB.log.unshift({id:perfUid(),ts:new Date().toISOString(),type,action,detail,user_id:perfSESSION?.id||'system',user_nome:perfSESSION?.nome||'Sistema'});
  if(perfDB.log.length>1000)perfDB.log=perfDB.log.slice(0,1000);
  perfSaveDB();
}
...
function perfOpenModal(id){document.getElementById(id).classList.add('open');}
function perfCloseModal(id){document.getElementById(id).classList.remove('open');}
document.querySelectorAll('.overlay').forEach(el=>el.addEventListener('click',e=>{if(e.target===el)el.classList.remove('open');}));

// ============================================================
// LOGIN
// ============================================================
function perfDoLogin_unused(){
  const login=document.getElementById('login-user').value.trim();
  const senha=document.getElementById('login-pass').value;
  const u=perfDB.users.find(u=>u.login===login&&u.senha===senha);
  if(!u){document.getElementById('login-error').style.display='block';return;}
  document.getElementById('login-error').style.display='none';
  perfSESSION=u;
  sessionStorage.setItem('pos_sess3',JSON.stringify({id:u.id,senha:u.senha}));
  perfAddLog('login','Login realizado',`Usuário: ${u.nome} | Perfil: ${u.role}`);
  document.getElementById('login-screen').style.display='none';
  document.getElementById('app').style.display='block';
  document.getElementById('sidebar-uname').textContent=u.nome;
  document.getElementById('sidebar-urole').textContent={admin:'Admin',manager:'Gestor',viewer:'Visualizador'}[u.role]||u.role;
  initMonthSelector();renderAll();
}
function perfDoLogout_unused(){
  SESSION=null;sessionStorage.removeItem('pos_sess3');
...
function initMonthSelector(){
  ...
  sel.innerHTML=months.map(k=>`<option value="${k}" ${k===prev?'selected':''}>${monthLabel(k)}</option>`).join('');
  // Garante que sempre tem um mês selecionado (o atual por padrão)
  if(!sel.value&&months.length)sel.value=months[0];
}

let _page='dashboard';
function showPage(n,f){perfShowPage(n,f);}
function perfShowPage(name,fromPage){
  const _pc=document.getElementById('perf-container');
  (_pc?_pc.querySelectorAll('.page'):document.querySelectorAll('.page')).forEach(p=>p.classList.remove('active'));
  (_pc?_pc.querySelectorAll('.nav-item'):document.querySelectorAll('.nav-item')).forEach(n=>n.classList.remove('active'));
  ...
  _page=name;
  if(fromPage){const bb=document.getElementById('back-btn');if(bb)bb.onclick=()=>perfShowPage(fromPage);}
  const pc=document.getElementById('perf-container');
  const ni=pc?pc.querySelectorAll('.nav-item'):[];
...
function perfRenderAll(){renderAll();}
function perfInitMonthSelector(){initMonthSelector();}
function renderAll(){
  initMonthSelector(); // garante que select sempre tem opções
...
// DASHBOARD
// ============================================================
function perfRenderDashboard(){
  const mk=curMonth();const funcs=perfDB.funcionarios;
  if(!funcs.length){
  ...
  const[y,m]=mk.split('-');
  const allRecs=[
    ...perfDB.falhas.filter(f=>{const fd=new Date(f.data+'T00:00:00');return fd.getFullYear()===parseInt(y)&&(fd.getMonth()+1)===parseInt(m);}).map(f=>({...f,kind:'fail'})),
    ...(perfDB.bonus||[]).filter(b=>{const bd=new Date(b.data+'T00:00:00');return bd.getFullYear()===parseInt(y)&&(bd.getMonth()+1)===parseInt(m);}).map(b=>({...b,kind:'bonus'}))
  ...
  document.getElementById('recent-fails').innerHTML=allRecs.length?allRecs.map(r=>{
    const fn=perfDB.funcionarios.find(fn=>fn.id===r.func_id);
...
function renderTabela(){
  const mk=curMonth();
  const ts=document.getElementById('filter-team');const teams=[...new Set(perfDB.funcionarios.map(f=>f.equipe).filter(Boolean))];
  ...
  let funcs=perfDB.funcionarios;if(ct)funcs=funcs.filter(f=>f.equipe===ct);
...
// FUNCIONÁRIO DETAIL
// ============================================================
function openDetail(id){window._did=id;window._fromPage=_page;perfShowPage('funcionario',_page);}
function renderFuncionarioDetail(id){
  const fn=perfDB.funcionarios.find(f=>f.id===id);
  ...
  const nota=perfDB.anotacoes[id+'_'+mk]||'';
  // Counts
...
function renderFuncionarios(){
  ...
  if(!perfDB.funcionarios.length){document.getElementById('employee-grid').innerHTML=`<div class="empty"><div class="empty-icon">👥</div><div class="empty-text">Nenhum funcionário.</div></div>`;return;}
  const mk=curMonth();
  document.getElementById('employee-grid').innerHTML=perfDB.funcionarios.map(f=>{
...
function openAddFuncionario(){
  ...
  perfOpenModal('overlay-func');
}
function openEditFuncionario(id){
  if(!perfCanEdit()){perfNotify('Sem permissão');return;}
  const f=perfDB.funcionarios.find(fn=>fn.id===id);if(!f)return;
...
function salvarFuncionario(){
  ...
  if(eid){
    const i=perfDB.funcionarios.findIndex(f=>f.id===eid);
    if(i>-1){perfDB.funcionarios[i]={...perfDB.funcionarios[i],...d};perfAddLog('func_edit',`Funcionário editado: ${nome}`,`Equipe: ${d.equipe} | Cargo: ${d.cargo}`);}
    perfNotify('Funcionário atualizado');
  } else {
    perfDB.funcionarios.push({id:perfUid(),...d});
    perfAddLog('func_add',`Funcionário adicionado: ${nome}`,`Equipe: ${d.equipe} | Cargo: ${d.cargo}`);
    perfNotify('Funcionário adicionado');
  }
  perfSaveDB();perfCloseModal('overlay-func');renderAll();
}
function deletarFuncionario(id){
  if(!perfIsAdmin()){perfNotify('Apenas admins podem remover');return;}
  const f=perfDB.funcionarios.find(fn=>fn.id===id);
  if(!confirm(`Remover ${f?.nome}?`))return;
  perfAddLog('func_delete',`Funcionário removido: ${f?.nome}`,'');
  perfDB.funcionarios=perfDB.funcionarios.filter(fn=>fn.id!==id);
  perfDB.falhas=perfDB.falhas.filter(fl=>fl.func_id!==id);
  perfDB.bonus=(perfDB.bonus||[]).filter(b=>b.func_id!==id);
  Object.keys(perfDB.anotacoes).forEach(k=>{if(k.startsWith(id+'_'))delete perfDB.anotacoes[k];});
  perfSaveDB();perfNotify('Removido');renderFuncionarios();
}
...
function openRegistrarFalha(pre){
  ...
  sel.innerHTML=perfDB.funcionarios.map(f=>`<option value="${f.id}">${f.nome}</option>`).join('');
...
function salvarFalha(){
  ...
  const ft=getFT(tipo);const fn=perfDB.funcionarios.find(f=>f.id===fid);
  const mk=getMonthKey(new Date(data+'T12:00:00'));
  const ep=calcEffectivePeso(fid,mk,tipo);
  perfDB.falhas.push({id:perfUid(),func_id:fid,data,tipo,peso:ep,motivo,by_user:perfSESSION.id});
  perfAddLog('fail_add',`Falha registrada: ${ft.label}`,`Funcionário: ${fn?.nome} | Data: ${perfFmtDate(data)} | Impacto: ${ep}pts | Motivo: ${motivo}`);
  perfSaveDB();perfCloseModal('overlay-falha');perfNotify('Falha registrada');loadFromFirebase(function(){renderAll();});
}
function deletarFalha(id){
  if(!perfCanEdit()){perfNotify('Sem permissão');return;}
  if(!confirm('Remover este registro?'))return;
  const f=perfDB.falhas.find(fl=>fl.id===id);
  const ft=getFT(f?.tipo);const fn=perfDB.funcionarios.find(fn=>fn.id===f?.func_id);
  perfAddLog('fail_delete',`Falha removida: ${ft?.label||f?.tipo}`,`Funcionário: ${fn?.nome} | Data: ${perfFmtDate(f?.data||'')} | Motivo: ${f?.motivo}`);
  perfDB.falhas=perfDB.falhas.filter(fl=>fl.id!==id);
  perfSaveDB();loadFromFirebase(function(){perfNotify('Removido');renderFuncionarioDetail(window._did);});
}
...
function salvarBonus(){
  ...
  const bt=getBT(tipo);const fn=perfDB.funcionarios.find(f=>f.id===fid);
  if(!perfDB.bonus)perfDB.bonus=[];
  perfDB.bonus.push({id:perfUid(),func_id:fid,data,tipo,valor:bt.valor,motivo,by_user:perfSESSION.id});
  perfAddLog('bonus_add',`Bônus registrado: ${bt.label}`,`Funcionário: ${fn?.nome} | Data: ${perfFmtDate(data)} | Valor: +${bt.valor}pts | Motivo: ${motivo}`);
  perfSaveDB();perfCloseModal('overlay-bonus');perfNotify('Bônus registrado!','green');loadFromFirebase(function(){renderAll();});
}
function deletarBonus(id){
  if(!perfCanEdit()){perfNotify('Sem permissão');return;}
  if(!confirm('Remover este bônus?'))return;
  const b=(perfDB.bonus||[]).find(bn=>bn.id===id);
  const bt=getBT(b?.tipo);const fn=perfDB.funcionarios.find(fn=>fn.id===b?.func_id);
  perfAddLog('bonus_delete',`Bônus removido: ${bt?.label||b?.tipo}`,`Funcionário: ${fn?.nome}`);
  perfDB.bonus=(perfDB.bonus||[]).filter(bn=>bn.id!==id);
  perfSaveDB();perfNotify('Bônus removido');renderFuncionarioDetail(window._did);
}
...
function salvarNota(fid,mk){
  ...
  perfDB.anotacoes[fid+'_'+mk]=txt;
  const fn=perfDB.funcionarios.find(f=>f.id===fid);
  perfAddLog('note_save','Anotação salva',`Funcionário: ${fn?.nome} | Mês: ${monthLabel(mk)}`);
  perfSaveDB();perfNotify('Anotação salva');
}
...
function renderHistorico(){
  ...
  us.innerHTML='<option value="">Todos os usuários</option>'+perfDB.users.map(u=>`<option value="${u.id}" ${u.id===cu?'selected':''}>${u.nome}</option>`).join('');
...
function exportarJSON(){
  ...
  perfAddLog('export','Exportação JSON','');perfSaveDB();perfNotify('JSON exportado');
}
...
function handleImport(e){
  ...
  const r=new FileReader();r.onload=ev=>{
    try{
      ...
      DB={...d,users:d.users||perfDB.users,log:d.log||[],bonus:d.bonus||[]};
      perfSaveDB();perfNotify('Dados importados');renderAll();
...
function exportarCSV(){
  ...
  perfDB.funcionarios.forEach(fn=>{
...
function resetarDados(){
  ...
  const me=perfDB.users.find(u=>u.id===perfSESSION.id);
  localStorage.removeItem('pos_db_v3');
  DB={users:[me],funcionarios:[],falhas:[],bonus:[],anotacoes:{},log:[]};
  perfSaveDB();perfNotify('Dados resetados');renderAll();
}

function renderPerformancePage(){
  if(!isAdmin1&&!currentProfile?.viewPerformance){
    return '<div style="display:flex;align-items:center;justify-content:center;height:60vh;flex-direction:column;gap:12px"><div style="font-size:48px">🔒</div><div style="color:#7a7a8a">Sem permissão.</div></div>';
  }
  const _mc=document.getElementById("perf-container");
  if(_mc){
    // Já renderizado — só recarregar dados
    perfLoadFromFirebase(function(){perfInit();perfInitMonthSelector();perfRenderAll();});
    return null;
  }
  // Primeira renderização
  perfLoadFromFirebase(function(){
    perfInit();
    const c=document.getElementById("perf-container");
    if(c){perfInitMonthSelector();perfRenderAll();}
  });
  return `<div id="perf-container" style="display:flex;height:calc(100vh - 56px);overflow:hidden">
<aside class="sidebar">
  <div class="logo"><div class="logo-title">PerformanceOS</div><div class="logo-sub">v3.0 · scoring inteligente</div></div>
  <div style="padding:4px 12px 12px"><button onclick="nav('dashboard');render()" style="width:100%;padding:8px;background:rgba(91,110,245,0.12);border:1px solid rgba(91,110,245,0.25);border-radius:8px;color:#5b6ef5;font-size:12px;cursor:pointer;font-family:inherit">← Voltar ao Sistema</button></div>
  <div class="nav-section">Principal</div>
  <div class="nav-item active" onclick="showPage('dashboard')"><span class="nav-icon">⬡</span><span>Dashboard</span></div>
  <div class="nav-item" onclick="showPage('tabela')"><span class="nav-icon">▦</span><span>Tabela Geral</span></div>
  <div class="nav-section">Ações</div>
  <div class="nav-item" onclick="openRegistrarFalha()"><span class="nav-icon">⊕</span><span>Registrar Falha</span></div>
  <div class="nav-item" onclick="openRegistrarBonus()"><span class="nav-icon">✦</span><span>Registrar Bônus</span></div>
  <div class="nav-item" onclick="showPage('historico')"><span class="nav-icon">◷</span><span>Histórico de Ações</span></div>
  <div class="nav-section">Config</div>
  <div class="nav-item" onclick="showPage('funcionarios')"><span class="nav-icon">◈</span><span>Funcionários</span></div>
  <div class="nav-item" onclick="showPage('regras')"><span class="nav-icon">⚖</span><span>Regras de Score</span></div>
  <div class="nav-item" onclick="showPage('exportar')"><span class="nav-icon">↗</span><span>Exportar</span></div>
  <div class="sidebar-user">
    <div class="sidebar-user-name" id="sidebar-uname"></div>
    <div class="sidebar-user-role" id="sidebar-urole"></div>
  </div>
</aside>
<main class="main">
  <div class="topbar">
    ...
    <div class="topbar-actions">
      <div class="month-selector">📅<select id="month-global" onchange="renderAll()"></select></div>
...
function attachContentEvents(){
  ...
  document.querySelectorAll(".toggle-view-performance").forEach(btn=>{
    btn.addEventListener("click",async()=>{
      ...
      const cur=!!users[uid2]?.viewPerformance;
      await dbSet(`users/${uid2}/viewPerformance`,!cur);
      toast("Performance "+(cur?"removida":"liberada")+" para "+(users[uid2]?.name||uid2),"success");
    });
  });
  document.querySelectorAll(".toggle-view-performance").forEach(btn=>{
    btn.addEventListener("click",async()=>{
