...
function loadFromFirebase(callback){
  ...
  fetch(_FB_URL+'/performance.json?auth='+_FB_TOKEN)
    ...
    .then(function(data){
      if(data&&!data.error&&data.funcionarios){
        DB=data;
        try{localStorage.setItem('pos_db_v3',JSON.stringify(DB));}catch(e){}
      }
      callback&&callback();
...
function initMonthSelector(){
  ...
  const prev=sel.value; // preserva mês atual se já havia seleção
...
function renderPerformancePage(){
  ...
  // Obtém token de autenticação Firebase para a REST API
  ...
  // Tenta obter token do usuário logado no Firebase Auth
  const _setIframe=(html)=>{
    setTimeout(()=>{ const f=document.getElementById('perf-iframe'); if(f) f.srcdoc=html; },30);
  };
  if(typeof auth!=="undefined"&&auth.currentUser){
    auth.currentUser.getIdToken().then(token=>_setIframe(_buildHtml(token))).catch(()=>_setIframe(_buildHtml("")));
  } else {
