...
let DB=loadDB();
let SESSION=null;
function saveDB(){try{localStorage.setItem('pos_db_v3',JSON.stringify(DB));}catch(e){}}
...
(function(){
  var s = (typeof _INJECTED_SESSION !== 'undefined') ? _INJECTED_SESSION
        : {id:'fallback',nome:'Admin',login:'admin',senha:'',role:'admin',cor:'#5b6ef5'};
  ...
  SESSION = u;
  // Executa direto — DOMContentLoaded nunca dispara em srcdoc
...
function renderPerformancePage(){
  if(!isAdmin1&&!currentProfile?.viewPerformance){
    return '<div style="display:flex;align-items:center;justify-content:center;height:60vh;flex-direction:column;gap:12px"><div style="font-size:48px">🔒</div><div style="color:#7a7a8a;font-size:14px">Sem permissão.</div></div>';
  }
