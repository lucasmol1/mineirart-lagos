...
(function(){
  ...
  if(!u){
    u={id:s.id,nome:s.nome,login:s.login,senha:s.senha||'__ext__',role:s.role,cor:s.cor||'#5b6ef5'};
...
function renderPerformancePage(){
  ...
  const _inj='<script>var _INJECTED_SESSION={id:"'+_pId+'",nome:"'+_pNome+'",login:"'+_pId+'",senha:"__ext__",role:"admin",cor:"#5b6ef5"};<\/script>';
  const _html=PERF_HTML_BASE.replace('</head>',_inj+'</head>');
  setTimeout(()=>{ const f=document.getElementById('perf-iframe'); if(f) f.srcdoc=_html; },30);
