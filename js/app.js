...
async function checkDeadlineAlerts(){
  ...
  for(const[id,t]of Object.entries(tasks)){
    ...
    // Remove notificação de atraso anterior desta mesma tarefa antes de criar a nova (evita acúmulo de repetidas)
    const prevNotifs=Object.entries((typeof window!=="undefined"&&window._myNotifs)||{}).filter(([,n])=>n.type==="overdue_task"&&n.taskId===id);
    for(const[pid]of prevNotifs)await dbRemove(`user_notifs/${currentUser.uid}/${pid}`);
