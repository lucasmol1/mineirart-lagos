...
function renderPerformancePage(){
  if(!isAdmin1&&!currentProfile?.viewPerformance){
    return '<div style="display:flex;align-items:center;justify-content:center;height:60vh;flex-direction:column;gap:12px"><div style="font-size:48px">🔒</div><div style="color:#7a7a8a;font-size:14px">Você não tem permissão para acessar este módulo.</div></div>';
  }
  const perfRole=isAdmin1?'admin':(currentProfile?.viewPerformance==="admin"?'admin':'manager');
  const perfNome=currentProfile?.name||'Usuário';
  const perfId=currentUser?.uid||'u_ext';
  const PERF_HTML=`<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
function curMonth(){return document.getElementById('month-global').value;}
...
// Garante que SESSION nunca é null ao usar as funções
function safeCanEdit(){return !SESSION||canEdit();}
function safeIsAdmin(){return !SESSION||isAdmin();}
...
function salvarFalha(){
  ...
  saveDB();closeModal('overlay-falha');notify('Falha registrada');renderAll();
}
function deletarFalha(id){
  ...
  saveDB();notify('Removido');renderFuncionarioDetail(window._did);
}
...
function salvarBonus(){
  ...
  saveDB();closeModal('overlay-bonus');notify('Bônus registrado!','green');renderAll();
}
...
const RCL={admin:'rb-admin',manager:'rb-manager',viewer:'rb-viewer'};
  document.getElementById('user-list').innerHTML=DB.users.map(u=>{
    ...
    return\`<div class="user-list-item">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        \${isSelf?\`<button class="btn-ghost btn-sm" onclick="">🔒 Senha</button>\`:''}
  ...
  if(eid){
    ...
    DB.users.push({id:uid(),nome,login,senha,role,cor:colors[DB.users.length%colors.length]});
...
// ============================================================
// INIT — controlado pelo sistema principal via postMessage
// ============================================================
window.addEventListener('message', function(e){
  if(!e.data||e.data.type!=='perf-login') return;
  const role = e.data.role || 'admin'; // default admin — acesso já foi validado externamente
  const nome = e.data.nome || 'Usuário';
  const id   = e.data.id   || 'u_ext';
  let u = DB.users.find(us=>us.id===id);
  if(!u){
    u = {id:id, nome:nome, login:id, senha:'__ext__', role:role, cor:'#5b6ef5'};
    DB.users.push(u); saveDB();
  } else {
    u.role=role; u.nome=nome; saveDB();
  }
  SESSION = u;
  document.getElementById('sidebar-uname').textContent = u.nome;
  document.getElementById('sidebar-urole').textContent = {admin:'Admin',manager:'Gestor',viewer:'Visualizador'}[u.role]||u.role;
...
// Avisa ao pai que está pronto para receber o login
window.parent.postMessage({type:'perf-ready'}, '*');
// Fallback: se postMessage demorar, inicia com role admin após 2s
setTimeout(()=>{ if(!SESSION){ SESSION={id:'fallback',nome:'Admin',login:'admin',senha:'',role:'admin',cor:'#5b6ef5'}; initMonthSelector(); renderAll(); } }, 2000);
`;
  // Remove listener antigo para evitar duplicatas ao navegar
  if(window._perfMsgHandler) window.removeEventListener('message',window._perfMsgHandler);
  window._perfMsgHandler=function(e){
    if(!e.data||e.data.type!=='perf-ready')return;
    const iframe=document.getElementById('perf-iframe');
    if(iframe&&iframe.contentWindow){
      iframe.contentWindow.postMessage({type:'perf-login',role:perfRole,nome:perfNome,id:perfId},'*');
    }
  };
  window.addEventListener('message',window._perfMsgHandler);
  setTimeout(()=>{
    const iframe=document.getElementById('perf-iframe');
    if(iframe) iframe.srcdoc=PERF_HTML;
  },30);
  return '<div style="width:100%;height:calc(100vh - 56px);overflow:hidden"><iframe id="perf-iframe" style="width:100%;height:100%;border:none;background:#0a0a0f" sandbox="allow-scripts allow-same-origin allow-downloads allow-modals"></iframe></div>';
}
        ...
        return`<div class="user-row" style="flex-wrap:wrap;gap:12px">
          ...
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px">
            ...
            ${u.id!==currentUser?.uid&&u.role!=="admin1"?`
              ...
              ${isAdmin1?`<div style="display:flex;align-items:center;gap:4px"><span style="font-size:10px;color:#7a7a8a">📊</span><select class="select-perf-role" data-uid="${u.id}" style="background:#13131a;border:1px solid ${u.viewPerformance?'#4ac8e8':'#2e2e3a'};color:${u.viewPerformance?'#4ac8e8':'#7a7a8a'};font-size:10px;border-radius:4px;padding:3px 6px;cursor:pointer;outline:none"><option value=\"\" ${!u.viewPerformance?'selected':''}>⬜ Perf: Sem acesso</option><option value=\"manager\" ${u.viewPerformance==='manager'?'selected':''}>👁 Perf: Gestor</option><option value=\"admin\" ${u.viewPerformance==='admin'?'selected':''}>⚙️ Perf: Admin</option></select></div>`:""}
...
function attachContentEvents(){
  ...
  document.querySelectorAll(".select-perf-role").forEach(sel=>{
    sel.addEventListener("change",async()=>{
      if(!isAdmin1)return;
      const uid2=sel.dataset.uid;
      const val=sel.value||false;
      await dbSet(`users/${uid2}/viewPerformance`,val);
      const label={"manager":"Gestor","admin":"Admin","":"Sem acesso"}[sel.value]||"Sem acesso";
      toast("Performance: "+label+" → "+(users[uid2]?.name||uid2),"success");
    });
  });
